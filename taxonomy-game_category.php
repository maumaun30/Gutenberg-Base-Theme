...
if ( $has_children ) {
    foreach ( $child_terms as $ct ) {
        $q = new WP_Query( [
            'post_type'      => 'game',
            'tax_query'      => [ [ 'taxonomy' => 'game_category', 'field' => 'term_id',
                                     'terms' => $ct->term_id, 'include_children' => true ] ],
            'posts_per_page' => 20,
            'orderby'        => 'date',
            'order'          => 'DESC',
        ] );
...
/* ---- Page --------------------------------------------------------- */
.gc-page { background:var(--bg-dark-3); min-height:100vh; color:#fff; }
...
.gc-orb--1 { top:0;    left:25%;  width:24rem; height:24rem;
  background:radial-gradient(circle,var(--color-primary),transparent); }
.gc-orb--2 { bottom:0; right:25%; width:24rem; height:24rem;
...
.gc-bc a { color:rgba(255,255,255,.5); text-decoration:none; transition:color .2s; }
.gc-bc a:hover { color:var(--color-primary); }
.gc-bc__sep { color:rgba(255,255,255,.25); }
.gc-bc__cur { color:var(--color-primary); }

/* Badge pill */
.gc-badge {
  display:inline-flex; align-items:center; gap:.5rem;
  padding:.75rem 1.5rem; border-radius:9999px; margin-bottom:2rem;
  background:rgba(247,29,194,.1); border:1px solid rgba(247,29,194,.3);
...
/* Heading */
.gc-hero h1 {
  font-family:'Bebas Neue',sans-serif; font-size:var(--text-h1);
  color:#fff; letter-spacing:.05em; line-height:1.1; margin-bottom:1.25rem;
}
.gc-hero h1 span { color:var(--color-primary); }
.gc-hero__desc {
  font-family:'Lexend',sans-serif; font-size:var(--text-lead);
...
.gc-stats {
  ...
  gap:1.25rem; max-width:48rem; margin:0 auto;
}
@media(min-width:640px){ .gc-stats { grid-template-columns:repeat(4,1fr); } }
.gc-stat {
...
.gc-stat svg      { width:1.5rem; height:1.5rem; margin:0 auto; color:var(--color-primary); }
.gc-stat__val     { font-family:'Bebas Neue',sans-serif; font-size:1.5rem;
                    color:#fff; letter-spacing:.05em; margin:.5rem 0 .25rem; }
.gc-stat__lbl     { font-family:'Lexend',sans-serif; font-size:.8rem;
...
.gc-filters__inner { max-width:80rem; margin:0 auto; padding:0 1.5rem; }
.gc-filters__lbl {
...
.gc-filters__row { display:flex; flex-wrap:wrap; gap:.625rem; }

.gc-pill {
  display:inline-flex; align-items:center; gap:.4rem;
  padding:.45rem 1.125rem; border-radius:9999px; cursor:pointer;
  background:var(--bg-dark-4); border:1px solid var(--border);
  font-family:'Outfit',sans-serif; font-size:.8rem; font-weight:600;
  color:rgba(255,255,255,.7); transition:all .22s ease;
  user-select:none;
}
.gc-pill:hover { border-color:var(--color-primary); color:var(--color-primary); }
.gc-pill.is-active {
  background:var(--color-primary); color:#000;
  border-color:var(--color-primary); box-shadow:var(--shadow-glow);
}
.gc-pill__count {
  background:rgba(0,0,0,.22); border-radius:9999px;
  padding:.1rem .45rem; font-size:.65rem; font-weight:700;
}
.gc-pill.is-active .gc-pill__count { background:rgba(0,0,0,.25); }

/* ------------------------------------------------------------------ */
/*  MAIN SECTION                                                       */
/* ------------------------------------------------------------------ */
.gc-section { background:var(--bg-dark-2); padding:var(--section-py) 0; }
.gc-container { max-width:80rem; margin:0 auto; padding:0 1.5rem; }

/* Collection block */
.gc-collection { margin-bottom:4rem; animation:fadeUp .4s ease both; }
.gc-collection.is-hidden { display:none; }
.gc-collection:last-child { margin-bottom:0; }
...
@media(min-width:768px){
  .gc-col-hd { flex-direction:row; align-items:center; justify-content:space-between; }
}
.gc-col-hd__left { display:flex; align-items:center; gap:.875rem; }
.gc-col-icon {
  width:3rem; height:3rem; border-radius:.75rem; flex-shrink:0;
  background:linear-gradient(135deg,var(--color-primary),var(--color-secondary));
  display:flex; align-items:center; justify-content:center;
}
.gc-col-icon svg   { width:1.375rem; height:1.375rem; color:#000; }
.gc-col-title      { font-family:'Bebas Neue',sans-serif; font-size:var(--text-h3);
                     color:#fff; letter-spacing:.05em; margin:0 0 .15rem; }
.gc-col-desc       { font-family:'Lexend',sans-serif; font-size:.875rem;
                     color:rgba(255,255,255,.55); margin:0; }

/* Arrow buttons */
.gc-slider-nav { display:flex; align-items:center; gap:.5rem; flex-shrink:0; }
.gc-arr {
...
.gc-arr:hover { background:var(--color-primary); border-color:var(--color-primary);
                color:#000; box-shadow:var(--shadow-glow); }
.gc-arr:disabled { opacity:.3; pointer-events:none; }
.gc-arr svg { width:1rem; height:1rem; }

/* Slider */
.gc-slider-wrap { position:relative; }
...
.gc-slider:active { cursor:grabbing; }
.gc-slider::-webkit-scrollbar { display:none; }
...
@media(min-width:480px)  { .gc-card { flex: 0 0 calc(33.333% - .75rem); } }
@media(min-width:768px)  { .gc-card { flex: 0 0 calc(25% - .85rem); } }
@media(min-width:1280px) { .gc-card { flex: 0 0 calc(20% - .9rem); } }

.gc-card:hover            { transform:translateY(-6px); box-shadow:var(--shadow-lg);
                             border-color:var(--border-strong); }
.gc-card:hover .gc-card__img  { transform:scale(1.07); }
.gc-card:hover .gc-card__name { color:var(--color-primary); }
.gc-card:hover .gc-card__glow { opacity:1; }

.gc-card__img-wrap { position:relative; aspect-ratio:3/4; overflow:hidden; }
.gc-card__img {
...
.gc-card__badge {
  ...
  background:rgba(247,29,194,.2); border:1px solid rgba(247,29,194,.45);
...
.gc-card__info { position:absolute; bottom:0; left:0; right:0; padding:1rem; }
.gc-card__name {
  font-family:'Bebas Neue',sans-serif; font-size:var(--text-h4);
  color:#fff; letter-spacing:.04em; margin:0; line-height:1.1;
  transition:color .3s;
}
.gc-card__glow {
...
.gc-empty {
  ...
  font-family:'Lexend',sans-serif; color:rgba(255,255,255,.4);
}
...
<div class="gc-filters" id="gc-filter-bar">
  <div class="gc-filters__inner">
    <p class="gc-filters__lbl">Browse Sub-categories</p>
...
<section class="gc-section">
  <div class="gc-container">
    ...
    <?php else : ?>
      <div class="gc-empty"><p>No games found in this category.</p></div>
    <?php endif; ?>
  </div>
</section>
<?php endif; ?>
...
(function () {
  ...
  document.querySelectorAll('.gc-arr').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      slider.scrollBy({ left: dir * cardScrollAmount(slider), behavior: 'smooth' });
    });
  });
  ...
  document.querySelectorAll('.gc-slider').forEach(function (slider) {
    syncArrows(slider);
    slider.addEventListener('scroll', function () { syncArrows(slider); }, { passive: true });
  });
