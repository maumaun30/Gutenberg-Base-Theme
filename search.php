 ...
 * Search results — games, promos, blog posts and pages.
 */
...
$games_q = new WP_Query([
  'post_type'      => ['game', 'promo', 'post', 'page'],
  'post_status'    => 'publish',
  's'              => $search_query,
...
/* Human-readable label for the result's post type badge */
function fm_search_type_label($post_type)
{
  $labels = [
    'game'  => 'Game',
    'promo' => 'Promo',
      ...
      <?php if ($games_q->have_posts()) : ?>
        ...
        <div class="fm-grid">
          <?php while ($games_q->have_posts()) : $games_q->the_post();
            ...
            $is_hot    = ($post_type === 'game' && function_exists('get_field')) ? (bool) get_field('is_hot') : false;
          ?>
